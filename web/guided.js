...
export const PROTOCOLS = {
  rate: { title: "Best stroke rate at a fixed pace", build: p => rateTest({ rates: p.rates, pace_s: p.pace, total_s: p.total * 60, warm_s: p.warm * 60 }),
    ...
    about: "Rows each rate twice, up and back down, at one pace. Heart rate at the end of each block, adjusted to the target power, finds the rate that costs you least." },
  ...
  drift: { title: "Drift test", build: p => driftTest({ watts: p.watts, total_s: p.total * 60 }),
    ...
    about: "Fixed power for a long row. Compares heart rate per watt in the second half with the first." },
  step: { title: "Step test", build: p => stepTest({ start_w: p.start, step_w: p.step, stages: p.stages, total_s: p.total * 60 }),
    ...
    about: "Stages of power up and back down again, all below zone 3, for your heart-rate-against-power line and, with your fitness settings, a VO2max estimate." },
  ...
  readiness: { title: "Readiness check", build: p => readinessCheck({ watts: p.watts }),
    ...
    about: "Optional. Five easy minutes at a fixed power; after three of these, each is compared with your usual, to flag a day that's out of line." },
  drill: { title: "Technique drill", build: p => drillSession({ drill: p.drill, target: p.target === "" || p.target == null ? null : p.target, repeats: p.repeats, total_s: p.total * 60 }),
    ...
    about: "Drill blocks with easy rowing between. Every ten drill strokes you hear how many hit the target." },
};
