...
/** A dragged size is kept as the part's own size, and for the parts a slider also governs (the
 *  tiles' width, the height of a chart) the slider is moved with it, so the chart inside grows
 *  too and the two controls never disagree. */
function onResized(el) {
  // only a size the user dragged counts: the corner writes it into the element's own style,
  // while the layout's sizes come from the variables, and reading those back would feed on itself
  const w = parseFloat(el.style.width), h = parseFloat(el.style.height);
  if (!w && !h) return;
  const l = loadLayout(); l.size = { ...(l.size || {}) };
  l.size[el.id] = { w: el.style.width || null, h: el.style.height || null };
  saveLayout(l);
  const set = (k, v) => { const s = $("ui_" + k); if (!s) return; const r = Math.round(v); if (s.value === String(r)) return; s.value = r; applyUI(); };
  const canvas = el.querySelector("canvas");
  // how much of the part isn't the chart, measured when the drag began (measuring it now would
  // use the height being dragged, and the new size would come out as the old one)
  const inner = v => v - (el._chrome ?? (el.clientHeight - (canvas ? canvas.clientHeight : 0)));
  if (el.id.startsWith("tile_")) { if (w) set("tile", Math.min(360, Math.max(110, w / (parseFloat(css("--text")) || 1)))); }
  else if (el.id.startsWith("trend")) { if (h && canvas) set("trend", Math.min(300, Math.max(70, inner(h)))); }
  else if (el.id === "panel_curve") {
    const m = $("main"), full = m ? m.clientWidth : window.innerWidth;
    if (w) set("w", Math.min(80, Math.max(30, 100 * w / full)));
    if (h && canvas) set("h", Math.min(640, Math.max(140, inner(h))));
  } else if (el.id === "panel_shape") {
    const m = $("main"), full = m ? m.clientWidth : window.innerWidth;
    if (w) set("w", Math.min(80, Math.max(30, 100 - 100 * w / full)));
  }
}
...
function applyArrange() {
  ...
  if (on && !applyArrange.observer) {
    ...
    for (const id of arrangeIds()) applyArrange.observer.observe($(id));
    // the browser can deliver a resize notice after the drag has ended (and holds them back
    // entirely while the page is in a hidden tab), so read the size here rather than wait for it
    ...
    document.addEventListener("pointerup", () => {
      for (const el of document.querySelectorAll(".arranging")) {
        if (!el.style.width && !el.style.height) continue;
        onResized(el);
        // where a slider governs the size (every tile's width, a chart's height), the part gives
        // its own up, so the one you dragged doesn't end up out of step with the rest
        const w = el.id.startsWith("tile_") || el.id === "panel_curve" || el.id === "panel_shape";
        const h = el.id.startsWith("trend") || el.id === "panel_curve";
        if ((w && el.style.width) || (h && el.style.height)) {
          const l = loadLayout(); l.size = l.size || {}; l.size[el.id] = l.size[el.id] || {};
          if (w) { el.style.width = ""; l.size[el.id].w = null; }
